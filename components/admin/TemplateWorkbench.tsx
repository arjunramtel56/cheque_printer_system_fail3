  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <button type="button" className="button small" onClick={handleCreate}>+ New template</button>
        ...
        <Link href="/admin/banks" className="text-button" style={{ fontSize: "0.82rem" }}>Manage banks →</Link>
        <Link href="/admin/calibration" className="text-button" style={{ fontSize: "0.82rem" }}>Calibration →</Link>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
          <thead>
            <tr style={{ borderBottom: "2px solid var(--border)" }}>
              ...
              <th style={{ textAlign: "left", padding: 8, color: "var(--text-secondary)" }}>Bank</th>
              <th style={{ textAlign: "left", padding: 8, color: "var(--text-secondary)" }}>Size</th>
              <th style={{ textAlign: "left", padding: 8, color: "var(--text-secondary)" }}>Orientation</th>
              <th style={{ textAlign: "left", padding: 8, color: "var(--text-secondary)" }}>Modes</th>
              <th style={{ textAlign: "left", padding: 8, color: "var(--text-secondary)" }}>Verification</th>
              <th style={{ textAlign: "left", padding: 8, color: "var(--text-secondary)" }}>Enabled</th>
              <th style={{ textAlign: "right", padding: 8, color: "var(--text-secondary)" }}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {templates.map((t) => (
              <tr key={t.id} style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <td style={{ padding: 8 }}>{t.bankName}</td>
                <td style={{ padding: 8 }}>{t.widthMm} × {t.heightMm} mm</td>
                <td style={{ padding: 8 }}>{t.orientation}</td>
                <td style={{ padding: 8, fontSize: "0.78rem" }}>{t.print?.supportedModes?.length ?? 0}</td>
                <td style={{ padding: 8 }}><VerificationBadge status={t.verification?.status ?? "unverified"} /></td>
                <td style={{ padding: 8 }}>{t.enabled ? "yes" : "no"}</td>
                <td style={{ padding: 8, textAlign: "right", whiteSpace: "nowrap" }}>
                  <Link href={`/admin/templates/${t.id}`} className="text-button" style={{ fontSize: "0.8rem" }}>Edit</Link>
                  <button type="button" className="text-button" style={{ fontSize: "0.8rem" }} onClick={() => handleDuplicate(t.id)}>Duplicate</button>
                  <button type="button" className="text-button" style={{ fontSize: "0.8rem", color: "var(--danger)" }} onClick={() => handleDelete(t.id)}>Delete</button>
                </td>
              </tr>
            ))}
            {templates.length === 0 && (
              <tr><td colSpan={8} style={{ padding: 16, textAlign: "center", color: "var(--text-muted)" }}>No templates.</td></tr>
            )}
          </tbody>
        </table>
      </div>
