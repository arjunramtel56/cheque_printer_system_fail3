...
function statusStyle(status: ChequeStatus) {
  if (status === "printed") return "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300";
  if (status === "draft") return "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300";
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main style={{ flex: 1, padding: "30px" }}>
          <div style={{ marginBottom: 20, position: "relative", maxWidth: 400 }}>
            ...
            <input type="search" placeholder="Search cheque number, payee or bank..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ width: "100%", padding: "10px 12px 10px 38px", borderRadius: "var(--radius-control)", border: "1px solid var(--border-strong)", background: "var(--surface)", color: "var(--text-primary)", fontSize: "0.88rem" }} />
          </div>

          <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
            {tabs.map((tab) => (
              <button key={tab} type="button" onClick={() => setFilter(tab)} style={{ padding: "8px 16px", borderRadius: 8, border: filter === tab ? "1px solid var(--brand-blue)" : "1px solid var(--border)", cursor: "pointer", fontSize: "0.85rem", fontWeight: 500, background: filter === tab ? "var(--brand-blue)" : "var(--surface)", color: filter === tab ? "#fff" : "var(--text-secondary)" }}>
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            {loading ? (
              <div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)", fontSize: "0.88rem" }}>Loading cheques...</div>
            ) : filtered.length === 0 ? (
              <div style={{ padding: 60, textAlign: "center", color: "var(--text-muted)" }}>
                <FileText size={36} style={{ opacity: 0.3, marginBottom: 12 }} />
                <p style={{ fontWeight: 600, color: "var(--text-secondary)", marginBottom: 4 }}>No cheques found.</p>
              </div>
            ) : (
                    ...
                    {filtered.map((cheque) => (
                      <tr key={cheque.id} style={{ borderBottom: "1px solid var(--border)" }}>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${statusStyle(cheque.status)}`}>{statusLabel(cheque.status)}</span>
                        </td>
                        <td style={{ padding: "12px 16px", textAlign: "center" }}>
                          <div style={{ display: "flex", justifyContent: "center", gap: 6 }}>
                            <button type="button" style={{ padding: "6px 8px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text-secondary)", cursor: "pointer" }} aria-label="Print" title="Print">
                              <Printer size={14} />
                            </button>
                            <button type="button" style={{ padding: "6px 8px", borderRadius: 6, border: "1px solid color-mix(in srgb, var(--danger) 30%, transparent)", background: "var(--surface)", color: "var(--danger)", cursor: "pointer" }} aria-label="Delete" title="Delete" onClick={() => handleDelete(cheque.id)}>
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </td>
